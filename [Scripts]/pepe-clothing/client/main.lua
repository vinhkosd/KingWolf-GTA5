...
AddEventHandler('Framework:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(2000, function()
        TriggerServerEvent("pepe-clothing:loadPlayerSkin")
...
Citizen.CreateThread(function()
    for k, v in pairs (Config.Stores) do
        ...
        if Config.Stores[k].shopType == "barber" then
            ...
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("Thợ hớt tóc")
            ...
            for k, v in pairs(Config.Stores) do
                ...
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 0.15, '~g~E~w~ - Cửa hàng quần áo')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 0.15, '~g~E~w~ - Tiệm hớt tóc')
                            end
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                if dist < 15 then
                    if not creatingCharacter then
                        ...
                        if dist < 2 then
                            if(PlayerData.job ~= nil and PlayerData ~= nil) then
                                if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                    DrawText3Ds(Config.ClothingRooms[k].x, Config.ClothingRooms[k].y, Config.ClothingRooms[k].z + 0.15, '~g~E~w~ - Trang phục')
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        ...
                                        Framework.Functions.TriggerCallback('pepe-clothing:server:getOutfits', function(result)
                                            openMenu({
                                                {menu = "roomOutfits", label = "Cài đặt trước", selected = true, outfits = Config.Outfits[PlayerData.job.name][gender]},
                                                {menu = "myOutfits", label = "Trang phục của tôi", selected = false, outfits = result},
                                                {menu = "character", label = "Tính cách", selected = false},
...
RegisterNetEvent('pepe-clothing:client:openOutfitMenu')
AddEventHandler('pepe-clothing:client:openOutfitMenu', function()
    Framework.Functions.TriggerCallback('pepe-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data)
    ...
    TriggerEvent('pepe-clothing:client:loadOutfit', data)
end)
...
RegisterNetEvent('pepe-clothing:client:openMenu')
AddEventHandler('pepe-clothing:client:openMenu', function()
    customCamLocation = nil
    Framework.Functions.TriggerCallback('pepe-clothing:server:getOutfits', function(result)
      openMenu({
          {menu = "character", label = "Tính cách", selected = true},
          {menu = "clothing", label = "Quần áo", selected = false},
          {menu = "myOutfits", label = "Trang phục của tôi", selected = false, outfits = result},
          {menu = "accessoires", label = "Phụ kiện", selected = false}
      })
    end)
...
RegisterNUICallback('TrackerError', function()
    TriggerEvent('chatMessage', "HỆ THỐNG", "error", "Bạn không thể loại bỏ một vòng đeo mắt cá chân..")
end)
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('pepe-clothing:saveOutfit', data.outfitName, model, skinData)
end)

RegisterNetEvent('pepe-clothing:client:reloadOutfits')
AddEventHandler('pepe-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('pepe-clothing:server:removeOutfit', data.outfitName, data.outfitId)
    TriggerEvent('chatMessage', "HỆ THỐNG", "warning", "Bạn đã gỡ bỏ "..data.outfitName.."!")
end)
...
AddEventHandler('pepe-clothing:client:CreateFirstCharacter', function()
...
RegisterNetEvent("pepe-clothing:loadSkin")
AddEventHandler("pepe-clothing:loadSkin", function(new, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('pepe-clothing:client:loadPlayerClothing', data, GetPlayerPed(-1))
    end)
end)

RegisterNetEvent('pepe-clothing:client:loadPlayerClothing')
AddEventHandler('pepe-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('pepe-clothing:client:loadOutfit')
AddEventHandler('pepe-clothing:client:loadOutfit', function(oData)
    local ped = GetPlayerPed(-1)
...
RegisterNetEvent("pepe-clothing:client:adjustfacewear")
AddEventHandler("pepe-clothing:client:adjustfacewear",function(type)
