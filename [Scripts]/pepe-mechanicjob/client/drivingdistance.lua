                ...
                if seat == ped then
                    if not CheckDone then
                        if vehiclemeters == -1 then
                            CheckDone = true
                            Framework.Functions.TriggerCallback('pepe-vehicletuning:server:IsVehicleOwned', function(IsOwned)
                                if IsOwned then
                ...
                if vehiclemeters ~= -1 then
                    if seat == ped then
                        if previousvehiclepos ~= nil then
                            ...
                            if DamageKey ~= nil then
                                ...
                                if chance == odd then
                                    for k, v in pairs(Config.Damages) do
                                        ...
                                        Framework.Functions.TriggerCallback('pepe-vehicletuning:server:SetPartLevel', function(result)
                                        end, plate, k, newDamage)
                                        -- TriggerServerEvent('pepe-vehicletuning:server:SetPartLevel', plate, k, newDamage)
                                    end
                                end
                            end
                            ...
                            TriggerEvent('pepe-hud:client:UpdateDrivingMeters', true, amount)
                            Framework.Functions.TriggerCallback('pepe-vehicletuning:server:UpdateDrivingDistance', function(result)
                            end, DrivingDistance[plate], plate)
                            -- TriggerServerEvent('pepe-vehicletuning:server:UpdateDrivingDistance', DrivingDistance[plate], plate)
                        end
                    else
                        if invehicle then
                            if DrivingDistance[plate] ~= nil then
...
RegisterNetEvent('pepe-vehicletuning:client:UpdateDrivingDistance')
AddEventHandler('pepe-vehicletuning:client:UpdateDrivingDistance', function(amount, plate)
