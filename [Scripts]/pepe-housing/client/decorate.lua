...
RegisterNetEvent("pepe-housing:client:decorate")
AddEventHandler("pepe-housing:client:decorate", function()
	Citizen.Wait(500)
	if IsInHouse then 
		if HasKey then 
...
RegisterNetEvent("pepe-housing:server:sethousedecorations")
AddEventHandler("pepe-housing:server:sethousedecorations", function(house, decorations)
...
function LoadDecorations(house)
	if Config.Houses[house]['Decorations'] == nil or next(Config.Houses[house]['Decorations']) == nil then
		Framework.Functions.TriggerCallback('pepe-housing:server:get:decorations', function(result)
...
function SaveDecorations()
	if Currenthouse ~= nil then
		...
		TriggerServerEvent("pepe-housing:server:save:decorations", Currenthouse, ObjectList)
	end
end
