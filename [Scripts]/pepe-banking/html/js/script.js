...
RefreshBank = function() {
    ...
    $.post('http://pepe-banking/GetPersonalBalance', JSON.stringify({}), function(Data){
        ...
        $('#private').html('<div class="account-card-container"><div style="padding-top: 10px;float:left;"><i class="fas fa-money-check"></i></div><div style="padding-left:60px;"><p id="account-name" class="account-name">'+ Data.Name +'</p><p id="account-number" class="account-number">' + Data.BankId + '</p><p id="account-amount" class="account-amount">Funds: €' + Data.Balance + '</p></div></div>');
    $.post('http://pepe-banking/GetPrivateAcounts', JSON.stringify({}), function(Accounts){
        for (const [key, value] of Object.entries(Accounts)) {
         Count = parseInt(Count) + parseInt(key)
...
$(document).on('click', '.account-card', function(e) {
    ...
    if ($(this).data('bankid') != '') {
        ...
        $('.bankid').html('<p>Tài khoản đã chọn: '+ ClickedData.bankid +'</p>')
        $('.bank-main-amount').html('<p>Số dư khả dụng</p>$'+ ClickedData.balance +'')
