	...
	while true do
		...
		if DoesEntityExist(closestObject) then
			...
			if GetDistanceBetweenCoords(pedCoords, sitCoords, true) <= 1.0 then
				DrawText3Ds(sitCoords, "~g~[E]~w~ Ngồi", 0.4)
			...
			if GetDistanceBetweenCoords(pedCoords, pickupCoords, true) <= 1.0 then
				DrawText3Ds(pickupCoords, "~g~[E]~w~ Đẩy xe lăn", 0.4)
...
Sit = function(wheelchairObject)
	wheelchairObject = NetWorkAnObject(wheelchairObject)
...
PickUp = function(wheelchairObject)
	...
	if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
		if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'anim@heists@box_carry@', 'idle', 3) then
			ShowNotification("Có ai đó đang đẩy xe lăn này!")
			return
		end
	end
	...
	local timeout = 2000
	while timeout > 0 and not NetworkHasControlOfEntity(wheelchairObject) do
		Wait(10)
		timeout = timeout - 10
	end

	if not NetworkHasControlOfEntity(wheelchairObject) then
		ShowNotification("Không thể sử dụng xe lăn, hãy thử sử dụng xe lăn khác!")
		-- wheelchairObject = ReNetWorkAnObject(wheelchairObject)
	else
		LoadAnim("anim@heists@box_carry@")
		print(wheelchairObject)
...
NetWorkAnObject = function(object)
	local netid = NetworkGetNetworkIdFromEntity(object)
	local object2 = NetworkGetEntityFromNetworkId(netid)

	if object2 == 0 then--the object is not networked -> recreate object and delete old object what was not networked
		print(object2)
		print(netid)
		LoadModel('prop_wheelchair_01')
		local wheelchair = CreateObject(GetHashKey('prop_wheelchair_01'), GetEntityCoords(object), true)
...
ReNetWorkAnObject = function(object)
	LoadModel('prop_wheelchair_01')
	local wheelchair = CreateObject(GetHashKey('prop_wheelchair_01'), GetEntityCoords(object), true)
	SetEntityAsNoLongerNeeded(object)
	SetEntityAsMissionEntity(object, true, true)
