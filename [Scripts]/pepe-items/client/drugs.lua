...
AddEventHandler('pepe-items:client:use:joint', function()
  Citizen.SetTimeout(1000, function()
    ...
    Framework.Functions.Progressbar("smoke-joint", "Một bi thôi nhé..", 2500, false, true, {
...
AddEventHandler('pepe-items:client:use:superjoint', function()
  Citizen.SetTimeout(1000, function()
    Framework.Functions.Progressbar("smoke-joint", "Đang sử dụng...", 2500, false, true, {
     disableMovement = false,
     disableCarMovement = false,
     disableMouse = false,
     disableCombat = true,
...
AddEventHandler('pepe-items:client:joint:effect', function()
  ...
  while OnWeed do
    ...
    if WeedTime > 0 then
      WeedTime = WeedTime - 1
      TriggerServerEvent('pepe-hud:server:remove:stress', math.random(2, 6))
      Citizen.Wait(1000)
    end
    if WeedTime == 0 then
       OnWeed = false
