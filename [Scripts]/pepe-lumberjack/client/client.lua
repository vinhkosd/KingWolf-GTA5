        ...
        if type(closeTo) == 'table' then
            while GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), closeTo.coords, true) <= 2.5 do
                ...
                if IsControlJustReleased(0, 38) then
                    ...
                    --     else
                    --         Framework.Functions.Notify("Bạn không có rìu chặt gỗ, hãy ra cửa hàng dụng cụ để mua rìu chặt gỗ!", "error")
                    --     end
                    -- end, 'wood-hachet')
                end
            end
...
Citizen.CreateThread(function()
	...
	local s1d7 = false
	if dist <= 2.0 then
	DrawText3D2(procesX, procesY, procesZ+0.1, "~g~E~s~ Xử lý gỗ")
		...
		Framework.Functions.Notify('Bạn không có gỗ.', 'error')
		end
		end	
	end
...
function SellKurczaki22()
    ...
    if(count == 0) then
    Framework.Functions.Progressbar("search_register", "Đang xử lý gỗ...", 5000, false, true, {disableMovement = true,
...
Citizen.CreateThread(function()
	...
	local s1d7 = false
	if dist <= 2.0 then
	DrawText3D2(sellX, sellY, sellZ+0.1, "~g~E~s~ Bán hàng")
		if IsControlJustPressed(0, Keys['E']) then 
		Framework.Functions.TriggerCallback('Framework:HasItem', function(result)
					hasBagd7 = result
					s1d7 = true
		...
		Framework.Functions.Notify('Bạn không có ván.', 'error')
		end
		end	
	end
...
function SellKurczaki()
    ...
    ----
    if(count == 0) then
    Framework.Functions.Progressbar("search_register", "Đang bán..", 5000, false, true, {disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                        disableInventory = true,
...
-------genera auto
RegisterNetEvent('pepe-jobwood:Auto')
AddEventHandler('pepe-jobwood:Auto', function(boatModel, plate)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
...
Citizen.CreateThread(function()
    while true do
		...
		-- 		local s1 = false
        --             Framework.Functions.Notify('Tôi đang chuẩn bị xe của bạn, nhiên liệu 100%.', 'success',7000)
        --             Framework.Functions.Progressbar("search_register", "Nhập mã thẻ..", 7000, false, true, {
        ...
        --                   TriggerServerEvent('pepe-jobwood:server:truck', model1, ClosestBerth)
        --                 end
        --             end, function()
