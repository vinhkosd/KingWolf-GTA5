...
Citizen.CreateThread(function()
	...
	local lapaniek = AddBlipForCoord(startX, startY, startZ)
		...
		AddTextComponentString('Bắt heo')
		...
		if dist <= 0.5 then
			...
			if IsControlJustPressed(0, Keys['E']) then -- "E"
				if isCatching then
					...
					TriggerServerEvent("pepe-pigjob:startChicken")
					SpawnPig()
				end
			end			
...
if share == true then
	...
	if dist <= 2.0 then
		...
		DrawText3D(pig1Coords.x, pig1Coords.y, pig1Coords.z+0.5, "~o~[E]~b~ Để bắt gà")
	...
	elseif dist2 <= 2.0 then
		...
		DrawText3D(pig2Coords.x, pig2Coords.y, pig2Coords.z+0.5, "~o~[E]~b~ Để bắt gà")
	...
	elseif dist3 <= 2.0 then
		...
		DrawText3D(pig3Coords.x, pig3Coords.y, pig3Coords.z+0.5, "~o~[E]~b~ Để bắt gà")
...
function TakePedToVehicleArea()
	...
	LoadDict('anim@heists@box_carry@')
		...
		if DoesEntityExist(vehicle) and vehDistance < 5.0 then
			...
			TriggerServerEvent("pepe-pigjob:getNewChicken")
...
function packingg(area)
	...
	Framework.Functions.TriggerCallback('Framework:HasItem', function(result)
		...
		if result then
			...
			TriggerServerEvent("pepe-pigjob:getpackedChicken",2)
...
function PortionChicken(position)
	...
	Framework.Functions.TriggerCallback('Framework:HasItem', function(result)
		if result then
			...
			pigOnTableProp = CreateObject(GetHashKey('prop_int_cf_chick_01'),-94.87, 6207.008, 30.08, true, true, true)

			SetEntityRotation(pigOnTableProp,90.0, 0.0, 45.0, 1,true)
			...
			TriggerServerEvent("pepe-pigjob:getcutChicken", 1)

			FreezeEntityPosition(GetPlayerPed(-1),false)

			DeleteEntity(pigOnTableProp)
			...
			pigOnTableProp = CreateObject(GetHashKey('prop_int_cf_chick_01'),-100.39, 6201.56, 29.99, true, true, true)
...
-- 				TriggerServerEvent("pepe-pigjob:sell", 3)
