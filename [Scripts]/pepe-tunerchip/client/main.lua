...
RegisterNUICallback('save', function(data)
    Framework.Functions.TriggerCallback('pepe-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            ...
            TriggerServerEvent('pepe-tunerchip:server:TuneStatus', GetVehicleNumberPlateText(veh), true)
        end
    end)
end)

RegisterNetEvent('pepe-tunerchip:server:TuneStatus')
AddEventHandler('pepe-tunerchip:server:TuneStatus', function()
    local ped = GetPlayerPed(-1)
    ...
    Framework.Functions.TriggerCallback('pepe-tunerchip:server:GetStatus', function(status)
        if status then
...
RegisterNetEvent('pepe-tunerchip:client:openChip')
AddEventHandler('pepe-tunerchip:client:openChip', function()
    local ped = GetPlayerPed(-1)
