...
RegisterCommand('fixtankwar', function()
	DoScreenFadeIn(1000)
end, false)
...
local PaintBallShop =  {
	{name="TankWar", id=73, x=-1590.945,y=2795.8085,z=17.071468,h=234.06784, typeM = "create"},
	-- table = {x = -1602.98, y = 2803.68, z = 17.1, h = 23.3},
	{name="TankWar", id=73, x = -1602.98, y = 2803.68, z = 17.1, h = 23.3, typeM = "join"},
	-- table = {x = -1597.11, y = 2809.17, z = 17.13, h = 46.29},
	{name="TankWar", id=73, -1597.11, y = 2809.17, z = 17.13, h = 46.29, typeM = "join"},
}

-- local outCoords = {x=-2324.017,y=3402.9125,z=30.571779,h=234.33169}
local outCoords = {x = -1584.96, y = 2789.32, z = 16.94, h = 37.91}
-- blueCoords = {x = -1878.92, y = 2808.02, z = 32.76, h = 153.61},
local blueCoords = {x = -1878.92, y = 2808.02, z = 32.76, h = 153.61} 
-- redCoords = {x = -2422.83, y = 3290.78, z = 32.79, h = 239.71},
local redCoords = {x = -2422.83, y = 3290.78, z = 32.79, h = 239.71}

local blipCoords = {x=-1590.945,y=2795.8085,z=17.071468,h=234.06784}
...
Citizen.CreateThread(function()
	...
	AddTextComponentString("TANK WAR")
                ...
                if(Vdist(pos.x, pos.y, pos.z, v.x, v.y, v.z) < 1.0)then
                    ...
                    if IsControlJustReleased(1, 51) then
						if v.typeM == "create" then
							...
							TriggerServerEvent("kingwolf-tankwar:GetAllSessions")
						end
                    end
        ...
        -- for k,v in ipairs(PaintBallShop) do
            if(Vdist(pos.x, pos.y, pos.z, mapRedOutCoords.x, mapRedOutCoords.y, mapRedOutCoords.z) < 5.0)then
                ...
                if(Vdist(pos.x, pos.y, pos.z, mapRedOutCoords.x, mapRedOutCoords.y, mapRedOutCoords.z) < 1.0)then
                    ...
                    incircleRed = true
                    if IsControlJustReleased(1, 51) then
						TriggerServerEvent("kingwolf-tankwar:LeaveTheGame",currentPartie)
                    end
...
RegisterNUICallback('validate', function(data, cb)
	...
	if reason == "" then
		TriggerServerEvent("kingwolf-tankwar:NewSession",data)
...
function JoinBlue(idx)
	TriggerServerEvent("kingwolf-tankwar:JoinBlue",idx)
end

function JoinRed(idx)
	TriggerServerEvent("kingwolf-tankwar:JoinRed",idx)
end

function StartTheGame(idx)
	TriggerServerEvent("kingwolf-tankwar:StartTheGame",idx)
end

RegisterNetEvent("kingwolf-tankwar:SendAllSessions")
AddEventHandler('kingwolf-tankwar:SendAllSessions', function(data)
...
RegisterNetEvent("kingwolf-tankwar:Notif")
AddEventHandler('kingwolf-tankwar:Notif', function(data,typeOfNotif)
	notification(data,typeOfNotif)
end)

_menuPool = MenuPool.New()
mainMenu = UIMenu.New("TANK WAR", "~b~PHÒNG CHỜ")
...
-- TriggerClientEvent("kingwolf-tankwar:GoForStartTheGame",v)
	...
	while true do
		...
		if HasPedBeenDamagedByWeapon(PlayerPedId(),GetHashKey(GunName),0) then
			...
			if not isCurrentlyOutInGame then
				...
				-- TriggerServerEvent("kingwolf-tankwar:GetSourceOfDamge")
				isCurrentlyOutInGame = true
				SetEntityHealth(PlayerPedId(),GetEntityHealth(PlayerPedId())+1.0)
				TriggerServerEvent("kingwolf-tankwar:addPoint",currentTeam,currentPartie)
				print("hit")
				FreezeEntityPosition(PlayerPedId(),true)
				isLockWaiting = true
				Wait(3000)
...
RegisterNetEvent('kingwolf-tankwar:GoToNextMancheMSG')
AddEventHandler('kingwolf-tankwar:GoToNextMancheMSG', function(winner)
...
RegisterNetEvent('kingwolf-tankwar:GoForTheNextGame')
AddEventHandler('kingwolf-tankwar:GoForTheNextGame', function(color,idx,manche)
	startNextGame(color,idx,manche)
end)


RegisterNetEvent('kingwolf-tankwar:GoForStartTheGame')
AddEventHandler('kingwolf-tankwar:GoForStartTheGame', function(color,idx,manche)
 	startGame(color,idx,manche)
end)

RegisterNetEvent('kingwolf-tankwar:GoToFinMatchMSG')
AddEventHandler('kingwolf-tankwar:GoToFinMatchMSG', function(winner)
	endTheGame(winner)
end)

RegisterNetEvent('kingwolf-tankwar:GoToLeaveMatchMSG')
AddEventHandler('kingwolf-tankwar:GoToLeaveMatchMSG', function(winner)
	leaveTheGame(winner)
end)
...
RegisterNetEvent('kingwolf-tankwar:Test')
AddEventHandler('kingwolf-tankwar:Test', function(color,idx,manche)
