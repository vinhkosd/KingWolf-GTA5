...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            var Locked = '<i class="fas fa-unlock"></i> Not yet started';
...
$(document).on('click', '#create-race', function(e){
    ...
    $.post('http://pepe-phone/IsAuthorizedToCreateRaces', JSON.stringify({}), function(data){
        if (data.IsAuthorized) {
            ...
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You have no rights to make racing tracks..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        ...
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You have to fill in a track name ..", "#1DA1F2");
    }
});
            ...
            $.post('http://pepe-phone/RaceDistanceCheck', JSON.stringify({
                ...
                if (InDistance) {
                    if (track !== undefined || track !== null) {
                        if (laps !== "") {
                            $.post('http://pepe-phone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    ...
                                    QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "No races can currently be made.", "#1DA1F2");
                                }
                            });
                        } else {
                        ...
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You have not chosen track..", "#1DA1F2");
                    }
                }
            })
        } else {
