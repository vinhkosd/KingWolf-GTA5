...
function CloseCreator(name, veh, price, financed)
	Citizen.CreateThread(function()
        ...
        TriggerServerEvent('pepe-sjperformance:server:setShowroomCarInUse', ClosestVehicle, false)
	end)
end
...
function SelectVehicle(vehicleData)
    TriggerServerEvent('pepe-sjperformance:server:setShowroomVehicle', vehicleData, ClosestVehicle)
    close()
end
                ...
                if VehicleData["shop"] == "pdm" then
                    ...
                    if SellDistance < 1 then
                        if not SellStarted then
                            DrawText3Ds(QB.QuickSell2.x, QB.QuickSell2.y, QB.QuickSell2.z, '~b~E~w~ - Verkoop voertuig voor ~g~€'..math.ceil(VehicleData["price"] / 100 * 60))
                            if IsControlJustPressed(0, Keys["E"]) then
                                SellStarted = true
                            end
                        else
                            DrawText3Ds(QB.QuickSell2.x, QB.QuickSell2.y, QB.QuickSell2.z, '~b~7~w~ - Bevestig / ~b~8~w~ - Annuleer')
                            if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustPressed(0, Keys["7"]) then
                                SellStarted = false
                                Framework.Functions.TriggerCallback('pepe-sjperformance:server:SellVehicle', function(SoldVehicle)
                                    if SoldVehicle then
                                        TriggerEvent("pepe-phone-new:client:BankNotify", "Er is €"..math.ceil(VehicleData["price"] / 100 * 60).." bijgeschreven")
                                        local veh = GetVehiclePedIsIn(ped)
                                        Framework.Functions.DeleteVehicle(veh)
                                    else
                                        Framework.Functions.Notify('Dit is niet jouw voertuig..', 'error')
                                    end
                ...
                if not QB.ShowroomVehicles2[ClosestVehicle].inUse then
                                ...
                                if isValidMenu(vehshop.currentmenu) then
                                    if selected then
                                        if IsControlJustPressed(1, 18) then
                                            if modelLoaded then
                                                TriggerServerEvent('pepe-sjperformance:server:setShowroomVehicle', button.model, ClosestVehicle)
                                            end
                                        end
                                    end
                                end
                    ...
                    if IsDisabledControlJustPressed(0, Keys["7"]) then
                        if buySure then
                            ...
                            TriggerServerEvent('pepe-sjperformance:server:buyShowroomVehicle', QB.ShowroomVehicles2[ClosestVehicle].chosenVehicle, class)
                            buySure = false
                        end
                    end
                    if IsDisabledControlJustPressed(0, Keys["8"]) then
                        Framework.Functions.Notify('Je hebt het voertuig niet gekocht', 'error', 3500)
                        buySure = false
                    end
...
RegisterNetEvent('pepe-sjperformance:client:setShowroomCarInUse')
AddEventHandler('pepe-sjperformance:client:setShowroomCarInUse', function(showroomVehicle, inUse)
    QB.ShowroomVehicles[showroomVehicle].inUse = inUse
end)

RegisterNetEvent('pepe-sjperformance:client:setShowroomVehicle')
AddEventHandler('pepe-sjperformance:client:setShowroomVehicle', function(showroomVehicle, k)
...
RegisterNetEvent('pepe-sjperformance:client:buyShowroomVehicle')
AddEventHandler('pepe-sjperformance:client:buyShowroomVehicle', function(vehicle, plate)
