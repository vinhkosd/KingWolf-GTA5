	...
	local lastselect = nil
		while self.visible do
			...
			if self.header then
				...
				drawMenuText(_U(menu.header:upper()),cfg.font,0,tx,ty,scale,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
			end
				...
				for i,btn in pairs(buttons) do
					if i >= menu.from and i <= menu.to then
						if menu.selected == i then
							...
							drawMenuText(_U(btn.name),cfg.font,0,tx,ty,scale,cfg.stext_color.r,cfg.stext_color.g,cfg.stext_color.b,cfg.stext_color.a)
							...
							elseif btn.price ~= nil then	
								if btn.price == 0 then
									...
									if type(btn.price) == "table" then
										if not btn.purchased then
											local label = ""
											for k, v in pairs(btn.price) do
												local itemInfo = Framework.Shared.Items[v.name]
												local itemName = itemInfo ~= nil and itemInfo["label"] or v.name
												label = label..v.amount.." x "..itemName..", "
											end
											drawRightMenuText("Cần "..label,cfg.font,0,x + (width/2.1),ty,scale,cfg.stext_color.r,cfg.stext_color.g,cfg.stext_color.b,cfg.stext_color.a)
										else
											drawRightMenuText("Đã mua",cfg.font,0,x + (width/2.1),ty,scale,cfg.stext_color.r,cfg.stext_color.g,cfg.stext_color.b,cfg.stext_color.a)
										end
									else
										if not btn.purchased then
							...
							drawMenuText(_U(btn.name),cfg.font,0,tx,ty,scale,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
							if btn.list then
								...
								if btn.price ~= 0 then
									if type(btn.price) == "table" then
										if not btn.purchased then
											...
											drawRightMenuText("Cần "..label,cfg.font,0,x + (width/2.1),ty,scale,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
										else
											drawRightMenuText("Đã mua",cfg.font,0,x + (width/2.1),ty,scale,cfg.stext_color.r,cfg.stext_color.g,cfg.stext_color.b,cfg.stext_color.a)
										end
									else
										if not btn.purchased then
											...
											drawRightMenuText("Đã mua",cfg.font,0,x + (width/2.1),ty,scale,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
										end
									end
								end
							elseif btn.menu ~= nil then
