...
Citizen.CreateThread(function()
	while true do
		...
		if NetworkIsSessionStarted() then
			TriggerEvent('pepe-multichar:client:open:select')
			return
		end
	end
end)

RegisterNetEvent('pepe-multichar:client:set:data')
AddEventHandler('pepe-multichar:client:set:data', function(Slot, CitizenId, Model, Skin)
    Config.Peds[Slot]['CitizenId'] = CitizenId
    Config.Peds[Slot]['Model'] = tonumber(Model)
    Config.Peds[Slot]['Skin'] = json.decode(Skin)
end)

RegisterNetEvent('pepe-multichar:client:open:select')
AddEventHandler('pepe-multichar:client:open:select', function()
    DoScreenFadeOut(10)
    Citizen.Wait(1000)
    TriggerServerEvent('pepe-multichar:server:set:data')
...
function CreatePeds()
    for k, v in pairs(Config.Peds) do
        ...
        exports['pepe-assets']:RequestModelHash(LoadModel)
        ...
        if v['Skin'] ~= nil then
            TriggerEvent('pepe-clothing:client:loadPlayerClothing', v['Skin'], CharPeds)
        end
...
RegisterNUICallback('GetCharacters', function(data, cb)
    Framework.Functions.TriggerCallback('pepe-multichar:server:get:characters', function(Characters)
        cb(Characters)
    end)
end)
...
RegisterNUICallback('ClickedCard', function(data)
    exports['pepe-assets']:RequestAnimationDict("friends@frj@ig_1")
...
RegisterNUICallback('SelectCharacter', function(data)
    ...
    Citizen.Wait(2500)
    exports['pepe-assets']:RequestAnimationDict("anim@mp_fm_event@intro")
    ...
    Citizen.SetTimeout(2000, function()
        ...
        RemoveFakePeds()
        ResetCitizenId()
        TriggerServerEvent('pepe-multichar:server:select:char', data.Citizenid)
    end)
end)
...
RegisterNUICallback('DeleteCharacter', function(data)
    ...
    Citizen.SetTimeout(1750, function()
        ...
        RemoveFakePeds()
        ResetCitizenId()
        TriggerServerEvent('pepe-multichar:server:delete:char', data.Citizenid)
    end)
end)
...
RegisterNUICallback('CreateNewChar', function(data)
    ...
    Citizen.SetTimeout(2000, function()
        ...
        RemoveFakePeds()
        ResetCitizenId()
        TriggerServerEvent('pepe-multichar:server:create:new:char', NewCharData)
    end)
end)
