...
AddEventHandler('Framework:Client:OnPlayerLoaded', function()
 Citizen.SetTimeout(1250, function()
     ...
     Citizen.Wait(350)
     Framework.Functions.TriggerCallback("pepe-emergencylights:server:get:config", function(config)
        Config = config
     end)
     LoggedIn = true
 end)
            ...
            if IsControlJustPressed(0, 36) then
               if GetVehicleClass(GetVehiclePedIsIn(GetPlayerPed(-1))) == 18 then
                   if not Config.UiOpend then
                       ...
                       TriggerServerEvent('pepe-emergencylights:server:set:sounds:disabled')
...
RegisterNetEvent('pepe-emergencylights:client:setup:first:time')
AddEventHandler('pepe-emergencylights:client:setup:first:time', function(Plate)
...
RegisterNetEvent('pepe-emergencylights:client:update:button')
AddEventHandler('pepe-emergencylights:client:update:button', function(Data, Plate)
    Config.ButtonData[Plate][Data.Type] = Data.State
end)

RegisterNetEvent('pepe-emergencylights:client:toggle:sounds')
AddEventHandler('pepe-emergencylights:client:toggle:sounds', function(Sender, State)
...
RegisterNetEvent('pepe-emergencylights:client:set:sounds:disabled')
AddEventHandler('pepe-emergencylights:client:set:sounds:disabled', function(Sender)
    local SelfPed = GetPlayerPed(GetPlayerFromServerId(Sender))
    local Vehicle = GetVehiclePedIsIn(SelfPed)
    ToggleMuteDefaultSiren(Vehicle, true)
end)
...
RegisterNUICallback('RegisterButton', function(data)
    ...
    TriggerServerEvent('pepe-emergencylights:server:update:button', data, Plate)
end)
...
RegisterNUICallback('SetSirens', function(data)
   TriggerServerEvent('pepe-emergencylights:server:toggle:sounds', data.Bool)
end)
...
function SetupEmergencyVehicle(Vehicle)
    ...
    TriggerServerEvent('pepe-emergencylights:server:setup:first:time', GetVehicleNumberPlateText(Vehicle))
    for i = 1, 15 do
