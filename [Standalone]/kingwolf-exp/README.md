...
* Download and extract the package: https://github.com/Mobius1/kingwolf-exp/archive/master.zip
* Rename the `kingwolf-exp-master` directory to `kingwolf-exp`
* Drop the `kingwolf-exp` directory into your `resources` directory on your server
* Import the `kingwolf-exp.sql` file into your db
* Add `ensure kingwolf-exp` in your `server.cfg`
...
```lua
exports.kingwolf-exp:ESXP_SetInitial(xp --[[ integer ]])
```

Set Rank for player. This will add the required XP to advance the player to the given rank.
```lua
exports.kingwolf-exp:ESXP_SetRank(rank --[[ integer ]])
```

Give player XP
```lua
exports.kingwolf-exp:ESXP_Add(xp --[[ integer ]])
```

Remove XP from player
```lua
exports.kingwolf-exp:ESXP_Remove(xp --[[ integer ]])
```
...
```lua
exports.kingwolf-exp:ESXP_GetXP()
```

Get player's current rank
```lua
-- Get rank from current XP
exports.kingwolf-exp:ESXP_GetRank()

-- or

-- Get rank from given XP
exports.kingwolf-exp:ESXP_GetRank(xp --[[ integer ]])

```

Get XP required to advance the player to the next rank
```lua
exports.kingwolf-exp:ESXP_GetXPToNextRank()
```

Get XP required to advance the player to the given rank
```lua
exports.kingwolf-exp:ESXP_GetXPToRank(rank --[[ integer ]])
```

Get max attainable XP
```lua
exports.kingwolf-exp:ESXP_GetMaxXP()
```

Get max attainable rank
```lua
exports.kingwolf-exp:ESXP_GetMaxRank()
```
...
Wait for `kingwolf-exp` to be ready for use
```lua
AddEventHandler("kingwolf-exp:ready", function(data --[[ table ]])
    local currentXP     = data.xp
    local currentRank   = data.rank
    local xPlayer       = data.player
    
    -- kingwolf-exp is ready for use
end)
```
...
```lua
AddEventHandler("kingwolf-exp:rankUp", function(newRank --[[ integer ]], previousRank --[[ integer ]])
    -- Do something when player ranks up
end)
```
Listen for rank-down event
```lua
AddEventHandler("kingwolf-exp:rankDown", function(newRank --[[ integer ]], previousRank --[[ integer ]])
...
To lock something to a rank you can listen for the `kingwolf-exp:rankUp` or `kingwolf-exp:rankDown` events:

Example of unlocking the minigun at rank 10:
```lua
AddEventHandler("kingwolf-exp:rankUp", function(newRank, previousRank)
    if newRank == 10 then
...
```lua
AddEventHandler("kingwolf-exp:rankDown", function(newRank, previousRank)
    if newRank < 10 then
