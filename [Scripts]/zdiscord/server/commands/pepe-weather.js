...
module.exports = {
    ...
    run: async (client, interaction, args) => {
        if (GetResourceState("pepe-weathersync") !== "started") return interaction.reply({ content: "This command requires Pepe's `pepe-weathersync` to work", ephemeral: false });
        if (args.blackout) {
            // doesn't give any option for true or false or feedback to which was done -.-
            emit("pepe-weathersync:server:toggleBlackout");
            ...
            emit("pepe-weathersync:server:setWeather", args.weather);
