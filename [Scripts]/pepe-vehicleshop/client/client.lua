...
AddEventHandler('Framework:Client:OnPlayerLoaded', function()
  Citizen.SetTimeout(1250, function()
      ...
      Citizen.Wait(100)
      Framework.Functions.TriggerCallback("pepe-vehicleshop:server:get:config", function(config)
        Config = config
      end)
      Citizen.Wait(15)
      isLoggedIn = true
              ...
              if Distance < 3.0 then
                 ...
                 elseif CanBuy then
                   ...
                   if IsDisabledControlJustPressed(0, 161) and CanBuy then
                       CanBuy = false
                       TriggerServerEvent('pepe-vehicleshop:server:buy:vehicle', Config.Vehicles[CurrentVehicle]['current-vehicle'], Framework.Shared.Vehicles[Config.Vehicles[CurrentVehicle]['current-vehicle']]['Price'])
                   end
       ...
       local Distance = GetDistanceBetweenCoords(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, Config.Sell['X'], Config.Sell['Y'], Config.Sell['Z'], true)
         if Distance < 5.0 and IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
           ...
           if IsControlJustReleased(0, 38) then
             ...
             Framework.Functions.TriggerCallback('pepe-vehicleshop:server:check:owner', function(IsOwner)
                 if IsOwner then
                    if Framework.Shared.Vehicles[VehicleModel:lower()]['Price'] ~= nil then
                      TriggerServerEvent('pepe-vehicleshop:server:sell:vehicle', Plate, Framework.Shared.Vehicles[VehicleModel:lower()]['Price'])
...
RegisterNetEvent('pepe-vehicleshop:client:spawn:bought:vehicle')
AddEventHandler('pepe-vehicleshop:client:spawn:bought:vehicle', function(VehicleName, Plate)
...
RegisterNetEvent('pepe-vehicleshop:client:set:vehicles')
AddEventHandler('pepe-vehicleshop:client:set:vehicles', function(RandomSport, RandomSedan, RandomMotor, RandomMuscle, RandomVans, RandomAddon)
  if InRange then
    RemoveVehicles()
  end
