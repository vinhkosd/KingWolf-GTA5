			...
			if Weapon ~= GetHashKey("WEAPON_UNARMED") and Weapon ~= GetHashKey("WEAPON_SNOWBALL") and Weapon ~= GetHashKey("WEAPON_STUNGUN") and Weapon ~= GetHashKey("WEAPON_PETROLCAN") and Weapon ~= GetHashKey("WEAPON_FIREEXTINGUISHER") and Weapon ~= GetHashKey("WEAPON_MOLOTOV") then
				...
				if ShotsFired > 5 and (CurrentStatusList == nil or CurrentStatusList["gunpowder"] == nil) then
					if math.random(1, 10) <= 7 then
						TriggerEvent("pepe-police:client:SetStatus", "gunpowder", 200)
					end
				end
		 ...
		 if PlayerJob.name ~= "police" and onDuty or PlayerJob.name == "police" and not onDuty then
			...
			if not SilentWeapon and not GunShotAlert then
				...
				if ShotsFiredAlert == 2 then
					...
					TriggerServerEvent('pepe-police:server:send:alert:gunshots', GetEntityCoords(PlayerPedId()), GunCategory, Framework.Functions.GetStreetLabel(), IsPedInAnyVehicle(PlayerPedId()))
				end
	...
	while true do
		...
		if isLoggedIn then
			if CurrentStatusList ~= nil and next(CurrentStatusList) ~= nil then
				...
				TriggerServerEvent("pepe-police:server:UpdateStatus", CurrentStatusList)
			end
	...
	while true do 
		...
		if CurrentCasing ~= nil then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, true) < 1.5 then
				DrawText3D(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, "~g~G~w~ - Vỏ đạn ~b~#"..Casings[CurrentCasing].type)
				if IsControlJustReleased(0, Config.Keys["G"]) then
					-- PickUp Event
					local StreetLabel = Framework.Functions.GetStreetLabel()
					local AmmoType = exports['pepe-weapons']:GetAmmoType(Casings[CurrentCasing].type)
					...
					TriggerServerEvent("pepe-police:server:AddEvidenceToInventory", 'casing', CurrentCasing, info)
				end
			end
		end
		...
		if CurrentBlooddrop ~= nil then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y, Blooddrops[CurrentBlooddrop].coords.z, true) < 1.5 then
				DrawText3D(Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y, Blooddrops[CurrentBlooddrop].coords.z, "~g~G~w~ - Mẫu máu")
				if IsControlJustReleased(0, Config.Keys["G"]) then
					-- PickUp Event
					local StreetLabel = Framework.Functions.GetStreetLabel()
					local info = {label = "Mẫu máu", type = "blood", street = StreetLabel:gsub("%'", ""), bloodtype = Blooddrops[CurrentBlooddrop].bloodtype}
					TriggerServerEvent("pepe-police:server:AddEvidenceToInventory", 'blood', CurrentBlooddrop, info)
				end
			end
		end
		...
		if CurrentFingerprint ~= nil then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y, Fingerprints[CurrentFingerprint].coords.z, true) < 1.5 then
				DrawText3D(Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y, Fingerprints[CurrentFingerprint].coords.z, "~g~G~w~ - Vân tay")
				if IsControlJustReleased(0, Config.Keys["G"]) then
					-- PickUp Event
					local StreetLabel = Framework.Functions.GetStreetLabel()
					local info = {label = "Vân tay", type = "fingerprint", street = StreetLabel:gsub("%'", ""), fingerprint = Fingerprints[CurrentFingerprint].fingerprint}
					TriggerServerEvent("pepe-police:server:AddEvidenceToInventory", 'finger', CurrentFingerprint, info)
				end
			end
		end
		...
		if CurrentHair ~= nil then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Hairs[CurrentHair].coords.x, Hairs[CurrentHair].coords.y, Hairs[CurrentHair].coords.z, true) < 1.5 then
				DrawText3D(Hairs[CurrentHair].coords.x, Hairs[CurrentHair].coords.y, Hairs[CurrentHair].coords.z, "~g~G~w~ - Mẫu tóc")
				if IsControlJustReleased(0, Config.Keys["G"]) then
					-- PickUp Event
					local StreetLabel = Framework.Functions.GetStreetLabel()
					local info = {label = "Mẫu tóc", type = "hair", street = StreetLabel:gsub("%'", ""), hair = Hairs[CurrentHair].hair}
					TriggerServerEvent("pepe-police:server:AddEvidenceToInventory", 'hair', CurrentHair, info)
				end
			end
		end
		...
		if CurrentSlimeDrop ~= nil then 
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, SlimeDrops[CurrentSlimeDrop].coords.x, SlimeDrops[CurrentSlimeDrop].coords.y, SlimeDrops[CurrentSlimeDrop].coords.z, true) < 1.5 then
				DrawText3D(SlimeDrops[CurrentSlimeDrop].coords.x, SlimeDrops[CurrentSlimeDrop].coords.y, SlimeDrops[CurrentSlimeDrop].coords.z, "~g~G~w~ - Nước bọt")
				if IsControlJustReleased(0, Config.Keys["G"]) then
					-- PickUp Event
					local StreetLabel = Framework.Functions.GetStreetLabel()
					local info = {label = "Nước bọt", type = "slime", street = StreetLabel:gsub("%'", ""), slime = SlimeDrops[CurrentSlimeDrop].slime}
					TriggerServerEvent("pepe-police:server:AddEvidenceToInventory", 'slime', CurrentSlimeDrop, info)
				end
			end
		end
	else
...
RegisterNetEvent('pepe-police:client:AddBlooddrop')
AddEventHandler('pepe-police:client:AddBlooddrop', function(bloodId, bloodtype, coords)
    Blooddrops[bloodId] = {
		bloodtype = bloodtype,
		coords = {
			x = coords.x,
...
RegisterNetEvent('pepe-police:client:AddFingerPrint')
AddEventHandler('pepe-police:client:AddFingerPrint', function(fingerId, fingerprint, coords)
...
RegisterNetEvent('pepe-police:client:AddHair')
AddEventHandler('pepe-police:client:AddHair', function(hairId, hair, coords)
    Hairs[hairId] = {
		hair = hair,
		coords = {
			x = coords.x,
...
RegisterNetEvent('pepe-police:client:AddSlime')
AddEventHandler('pepe-police:client:AddSlime', function(slimeId, slime, coords)
    SlimeDrops[slimeId] = {
		slime = slime,
		coords = {
			x = coords.x,
...
RegisterNetEvent('pepe-police:client:AddCasing')
AddEventHandler('pepe-police:client:AddCasing', function(casingId, weapon, coords, serie)
    Casings[casingId] = {
		type = weapon,
...
RegisterNetEvent("pepe-police:client:RemoveDnaId")
AddEventHandler("pepe-police:client:RemoveDnaId", function(Type, DnaId)
	if Type == 'casing' then
	 Casings[DnaId] = nil
...
RegisterNetEvent('pepe-police:client:SetStatus')
AddEventHandler('pepe-police:client:SetStatus', function(statusId, time)
...
function DropBulletCasing(Weapon)
 ...
 TriggerServerEvent("pepe-police:server:CreateCasing", Weapon, Coords)
 Citizen.Wait(300)
end
