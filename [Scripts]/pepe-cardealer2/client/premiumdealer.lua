...
RegisterNetEvent('pepe-cardealer2:client:sync:premium:dealer')
AddEventHandler('pepe-cardealer2:client:sync:premium:dealer', function(Slot, ConfigData)
    Config.PremiumDealerSpots[Slot] = ConfigData
    RefreshCars()
end)

RegisterNetEvent('pepe-cardealer2:client:set:premium:details')
AddEventHandler('pepe-cardealer2:client:set:premium:details', function(Vehicle, Price, Display, Stock)
    Config.PremiumVehicleDetails[Vehicle] = {['Price'] = Price, ['Display'] = Display, ['Stock'] = Stock}
end)

RegisterNetEvent('pepe-cardealer2:client:can:buy:vehicle')
AddEventHandler('pepe-cardealer2:client:can:buy:vehicle', function(VehicleSlot)
    CurrentBuyVehicle = VehicleSlot
end)

RegisterNetEvent('pepe-cardealer2:client:set:stock')
AddEventHandler('pepe-cardealer2:client:set:stock', function(StockData, Slot)
    Config.PremiumDealerSpots[Slot]['Stock'] = StockData
end)

RegisterNetEvent('pepe-cardealer2:client:spawn:car:premium')
AddEventHandler('pepe-cardealer2:client:spawn:car:premium', function(VehicleName, Plate)
...
RegisterNetEvent('pepe-cardealer2:client:sell:closest:vehicle')
AddEventHandler('pepe-cardealer2:client:sell:closest:vehicle', function()
    if CurrentVehicle ~= nil then
        ...
        if Player ~= -1 and Distance < 2.5 then
            Framework.Functions.Progressbar("lockpick-door", "Voertuig Verkopen..", 5500, false, false, {
                ...
                TriggerServerEvent('pepe-cardealer2:server:sell:closest', GetPlayerServerId(Player), CurrentVehicle)
...
RegisterNUICallback('SetCarInSlot', function(data)
    TriggerServerEvent('pepe-cardealer2:server:set:premium:data', data.slot, data.price, data.model, data.stock)
end)
                ...
                if CurrentBuyVehicle ~= nil and NearCar then
                    ...
                    if Distance <= 2.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            TriggerServerEvent('pepe-cardealer2:server:buy:current:vehicle', CurrentBuyVehicle)
...
RegisterNUICallback('SetSlotVehicle', function(data)
    if Config.PremiumVehicleDetails[data.model] ~= nil then
        TriggerServerEvent('pepe-cardealer2:server:set:premium:data', data.slot, Config.PremiumVehicleDetails[data.model]['Price'], data.model, Config.PremiumVehicleDetails[data.model]['Display'], Config.PremiumVehicleDetails[data.model]['Stock'])
    end
end)
