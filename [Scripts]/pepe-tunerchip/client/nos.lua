        ...
        if IsInVehicle then
            ...
            if VehicleNitrous[Plate] ~= nil then
                if VehicleNitrous[Plate].hasnitro then
                        ...
                        Citizen.CreateThread(function()
                            while NitrousActivated do
                                if VehicleNitrous[Plate].level - 1 ~= 0 then
                                    ...
                                    TriggerEvent('pepe-hud:client:UpdateNitrous', VehicleNitrous[Plate].hasnitro,  VehicleNitrous[Plate].level, true)
                                else
                    ...
                    if IsControlJustReleased(0, Keys["LEFTCTRL"]) then
                        if NitrousActivated then
                            ...
                            StopScreenEffect("RaceTurbo")
                            TriggerEvent('pepe-hud:client:UpdateNitrous', VehicleNitrous[Plate].hasnitro,  VehicleNitrous[Plate].level, false)
                            NitrousActivated = false
                        end
                    end
                end
                ...
                if not nosupdated then
                    TriggerEvent('pepe-hud:client:UpdateNitrous', false, nil, false)
                    nosupdated = true
                end
            end
        else
