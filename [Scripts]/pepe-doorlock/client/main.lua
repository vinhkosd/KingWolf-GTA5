		    	...
		    	if IsAdvanced then
		    		exports['pepe-lockpick']:OpenLockpickGame(function(Success)
		    			if Success then
		    				...
		    				Framework.Functions.Notify('Thất bại..', 'error', 2500)
		    			end
					end)
				else
					Framework.Functions.Notify('Bạn phải sử dụng lockpick nâng cao..', 'error', 2500)
				end
			else
				Framework.Functions.Notify('Không thể phá khóa cửa này..', 'error', 2500)
			end
		end
	end
end)

RegisterNetEvent('pepe-items:client:use:drill')
AddEventHandler('pepe-items:client:use:drill', function()
	  			...
	  			if not DoorValue['Pickable'] then
					Framework.Functions.TriggerCallback("pepe-bankrobbery:server:HasItem", function(HasItem)
						if HasItem then
							...
							TriggerEvent("pepe-inventory:client:ItemBox", Framework.Shared.Items['drill-bit'], "remove")
	  						if not exports['pepe-assets']:GetPropStatus() then
							...
							Framework.Functions.Notify('Bạn thiếu drill-bit..', 'error', 2500)
						end
					end, 'drill-bit')
	  		    else
	  		    	Framework.Functions.Notify('Không thể phá khóa cửa này..', 'error', 2500)
...
function LockpickFinish(Success)
 	if Success then
		...
		Framework.Functions.Progressbar("lockpick-door", "Đang phá khóa..", lockpickTime, false, true, {
			...
			ClearPedTasks(GetPlayerPed(-1))
			Framework.Functions.Notify('Thành công!', 'success', 2500)
			SetDoorLock(DoorValue, DoorKey)
		end, function() -- Cancel
			openingDoor = false
			ClearPedTasks(GetPlayerPed(-1))
