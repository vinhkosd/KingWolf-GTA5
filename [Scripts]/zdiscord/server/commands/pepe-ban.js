...
module.exports = {
    ...
    run: async (client, interaction, args) => {
        ...
        emit("pepe-admin:server:ban", player, time, reason);
        */
        const bantime = args.time < 2147483647 ? (args.time + Math.floor(Date.now() / 1000)) : 2147483647;
        global.exports.oxmysql.execute("INSERT INTO server_bans (name, license, discord, ip, reason, expire, bannedby) VALUES (?, ?, ?, ?, ?, ?, ?)", [
            GetPlayerName(args.id),
            client.Framework.Functions.GetIdentifier(args.id, "license"),
            client.Framework.Functions.GetIdentifier(args.id, "discord"),
            client.Framework.Functions.GetIdentifier(args.id, "ip"),
            args.reason,
            bantime,
        ...
        emit("pepe-log:server:CreateLog", "bans", "Player Banned", "red", `${GetPlayerName(args.id)} was banned by ${interaction.member.displayName} for ${args.reason}`, true);
        if (bantime >= 2147483647) {
            DropPlayer(args.id, `You have been banned:\n${args.reason}\n\nYour ban is permanent.\n🔸 Check our Discord for more information: ${client.Framework.Config.Server.discord}`);
        } else {
