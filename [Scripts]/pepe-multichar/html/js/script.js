...
$(document).on('click', '.multichar-card', function(e) {
 e.preventDefault();
  if (CanChoose) {
    $.post('http://pepe-multichar/Click', JSON.stringify({}));
    if (SelectedChar !== null) {
      ...
      $.post('http://pepe-multichar/ClickedCard', JSON.stringify({Slot: ClickedData.Slot}));
      if (ClickedData.CitizenId == "" || ClickedData.CitizenId == undefined) {
       $('.play-select-container').animate({bottom: "-20vh"}, 1000)
...
$(document).on('click', '.play-button', function(e) {
    ...
    if (CanChoose) {
        if (ClickedData.CitizenId != "" && ClickedData.Slot != null) {
          $.post('http://pepe-multichar/Click', JSON.stringify({}));
          $.post('http://pepe-multichar/SelectCharacter', JSON.stringify({Citizenid: ClickedData.CitizenId, Slot: ClickedData.Slot}));
          CloseCharScreen()
        }
    }
});
...
$(document).on('click', '.delete-button', function(e) {
    ...
    if (CanChoose) {
        if (ClickedData.CitizenId != "" && ClickedData.Slot != null) {
          $.post('http://pepe-multichar/Click', JSON.stringify({}));
          $.post('http://pepe-multichar/DeleteCharacter', JSON.stringify({Citizenid: ClickedData.CitizenId, Slot: ClickedData.Slot}));
          CloseCharScreen()
        }
    }
});
...
$(document).on('click', '.submit-button', function(e) {
    ...
    if (CanChoose) {
        ...
        if (CharName != "" && CharLastName != "" && CharBirthdate != "") {
          $.post('http://pepe-multichar/Click', JSON.stringify({}));
          $.post('http://pepe-multichar/CreateNewChar', JSON.stringify({FirstName: CharName, LastName: CharLastName, BirthDate: CharBirthdate, Gender: ResultGender, Slot: ClickedData.Slot}));
          CloseCharScreen()
        }
    }
});
...
SetupChars = function() {
  ...
  $.post('http://pepe-multichar/GetCharacters', JSON.stringify({}), function(Characters){
     for (const [key, value] of Object.entries(Characters)) {
          $("#char-"+value.CharSlot).html('');
          $("#char-"+value.CharSlot).data('citizenid', value.Citizenid)
          $("#char-"+value.CharSlot).html('<p>Naam: '+ value.Name +'</p><p>Baan: '+ value.Job +'</p><p>BSN: '+ value.Citizenid +'</p><span class="tooltiptext">Selecteren</span>');
      }
  });
}
...
ResetJs = function() {
 $("#char-1").html('<p>Nieuw Karakter</p><span class="tooltiptext">Aanmaken</span>');
 $("#char-2").html('<p>Nieuw Karakter</p><span class="tooltiptext">Aanmaken</span>');
 $("#char-3").html('<p>Nieuw Karakter</p><span class="tooltiptext">Aanmaken</span>');
...
CloseCharScreen = function() {
  ...
  $.post('http://pepe-multichar/CloseNui', JSON.stringify({}));
}
