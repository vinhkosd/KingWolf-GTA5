                        ...
                        if dist < 30 then
                            ...
                            if dist < 0.6 then
                                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                                    ...
                                    if IsControlJustPressed(0, Keys["E"]) then
                                        Framework.Functions.TriggerCallback('pepe-ammunation:server:getCops', function(cops)
                            ...
                            if storeDist < 2 then
                                if not firstAlarm then
                                    if validWeapon() then
                                        TriggerServerEvent('pepe-ammunation:server:PoliceAlertMessage', "Verdachte situatie", pos, true)
                                        firstAlarm = true
                                    end
                                end
                            end
...
function smashVitrine(k)
    ...
    Framework.Functions.Progressbar("smash_vitrine", "Vitrine aan het inslaan..", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
        ...
        TriggerServerEvent('pepe-ammunation:server:setVitrineState', "isOpened", true, k)
        TriggerServerEvent('pepe-ammunation:server:setVitrineState', "isBusy", false, k)
        TriggerServerEvent('pepe-ammunation:server:vitrineReward')
        TriggerServerEvent('pepe-ammunation:server:setTimeout')
        TriggerServerEvent('pepe-ammunation:server:PoliceAlertMessage', "Ammu Nation overval", plyCoords, false)
        smashing = false
    ...
    TriggerServerEvent('pepe-ammunation:server:setVitrineState', "isBusy", true, k)
...
RegisterNetEvent('pepe-ammunation:client:setVitrineState')
AddEventHandler('pepe-ammunation:client:setVitrineState', function(stateType, state, k)
    Config.Locations[k][stateType] = state
end)

RegisterNetEvent('pepe-ammunation:client:setAlertState')
AddEventHandler('pepe-ammunation:client:setAlertState', function(bool)
    robberyAlert = bool
end)

RegisterNetEvent('pepe-ammunation:client:PoliceAlertMessage')
AddEventHandler('pepe-ammunation:client:PoliceAlertMessage', function(title, coords, blip)
    if blip then
        TriggerEvent('pepe-police:client:AddPoliceAlert', {
            timeOut = 5000,
            alertTitle = title,
            details = {
                [1] = {
