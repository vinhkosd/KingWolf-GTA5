...
$(document).on('click', '.car-slot', function(e) {
    ...
    $.post('http://pepe-cardealer2/Click', JSON.stringify({}))
...
$(document).on('click', '.subit-button', function(e) {
    ...
    $.post('http://pepe-cardealer2/Click', JSON.stringify({}))
    $.post('http://pepe-cardealer2/SetSlotVehicle', JSON.stringify({slot: CurrentData.Slot, model: SelectedModel}))
...
$(document).on('click', '.menu-item', function(e) {
    ...
    if (MenuType != CurrentMenu) {
      ...
      $.post('http://pepe-cardealer2/ErrorSound', JSON.stringify({}))
    }
});

SetupStock = function() {
  $.post('http://pepe-cardealer2/GetStockVehicles', JSON.stringify({}), function(CarData){
...
SetupCarSlots = function() {
  $.post('http://pepe-cardealer2/GetDisplayVehicles', JSON.stringify({}), function(CarData){
    for (const [key, value] of Object.entries(CarData)) {
...
CloseTablet = function() {
  $('.main-container').animate({"left": "-180vh"}, 250, function() {
    ...
    $.post('http://pepe-cardealer2/CloseNui', JSON.stringify({}))
  })
}
