...
Framework.Commands.Add("giveamount", "Đưa đạn cho vũ khí những gì bạn đang sửa chữa", {{name="Hoeveelheid", help="Số đạn?"}}, true, function(source, args)
    if args[1] ~= nil then
            ...
            if WeaponSlot.info.quality ~= nil then
                for i = 1, RepeatAmount, 1 do
                    if WeaponSlot.info.quality - DecreaseAmount > 0 then
                        ...
                        TriggerClientEvent('Framework:Notify', src, "Vũ khi đã hỏng.", "error")
                        break
                    end
                end
            else
    ...
    if Inventory[CurrentWeaponData.slot] ~= nil then
        if Inventory[CurrentWeaponData.slot].info.attachments ~= nil and next(Inventory[CurrentWeaponData.slot].info.attachments) ~= nil then
            ...
            if not HasAttach then
                ...
                TriggerClientEvent("Framework:Notify", src, "You already have a "..AttachmentData.label:lower().." attached on your weapon.", "error", 3500)
            end
        else
