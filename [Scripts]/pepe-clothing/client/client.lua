...
function OpenMenu(name)
    ...
    if isAllowed then
        ...
        TriggerEvent("DoLongHudText", "Bạn không được chào đón ở đây!");
    end
end
...
AddEventHandler('hairDresser:ToggleHair', function()
   ...
   for _, item in pairs(barberShops) do
        if not showBarberShopBlips then
            ...
            AddTextComponentString("Tiệm hớt tóc")
...
Citizen.CreateThread(function()
	...
	while true do
        ...
        elseif nearcloth < 3.0 then
            menu = {"clothesmenu", "Nhấn ~g~K~s~ để thay đổi quần áo $ "..StoreCost}
        elseif neartat < 5.0 then
            menu = {"tattoomenu", "Nhấn ~g~K~s~ để đặt hình xăm $ "..StoreCost}
        elseif nearbarber < 5.0 then
            menu = {"barbermenu", "Nhấn ~g~K~s~ Cho thợ làm tóc $ "..StoreCost}
...
AddEventHandler("raid_clothes:setclothes", function(data,alreadyExist)
    ...
    local function setDefault()
        Citizen.CreateThread(function()
            ...
            TriggerEvent("customNotification","Có vẻ như bạn đã chọn một nhân vật, được thực hiện tốt! Bạn đã đặt một khách sạn trong thành phố, nhấn M để lấy điện thoại di động của bạn để lấy taxi, hoặc, nếu bạn thấy một, nhấn F để nhảy vào nó (điều này hoạt động ở khắp mọi nơi). Khi vào taxi, hãy đánh dấu vị trí trên bản đồ nơi bạn muốn đi miễn phí! (Khách sạn 1).")
            ...
            TriggerEvent("customNotification","Nhìn có khách sạn! Chúng ta hãy vào trong để xem nó trông như thế nào, F1 đi vào bên trong.")
        end)
    end
...
RegisterNetEvent("pepe-clothing:stealOutfit")
AddEventHandler("pepe-clothing:stealOutfit", function(cheater)
    TriggerServerEvent("pepe-clothing:letSteal", cheater, GetCurrentPed())
end)

RegisterNetEvent("pepe-clothing:letSteal")
AddEventHandler("pepe-clothing:letSteal", function(data)
    ForceOutfit = GetCurrentPed()
    LoadPed(data)
end)

RegisterNetEvent("pepe-clothing:restoreOutfit")
AddEventHandler("pepe-clothing:restoreOutfit", function()
    if ForceOutfit then
        LoadPed(ForceOutfit)
    end
end)