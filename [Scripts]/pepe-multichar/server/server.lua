...
Framework.Functions.CreateCallback("pepe-multichar:server:get:characters", function(source, cb)
...
RegisterServerEvent('pepe-multichar:server:set:data')
AddEventHandler('pepe-multichar:server:set:data', function()
    local src = source
    Framework.Functions.ExecuteSql(false, "SELECT * FROM characters_metadata WHERE `steam` = '"..GetPlayerIdentifiers(src)[1].."'", function(result)
        for k, v in pairs(result) do
            Framework.Functions.ExecuteSql(false, "SELECT * FROM `characters_skins` WHERE `citizenid` = '"..v.citizenid.."'", function(modelresult)
                if modelresult ~= nil and modelresult[1] ~= nil then
                    TriggerClientEvent('pepe-multichar:client:set:data', src, v.cid, v.citizenid, modelresult[1].model, modelresult[1].skin)
                else
                    TriggerClientEvent('pepe-multichar:client:set:data', src, v.cid, v.citizenid, nil, nil)
                end
            end)
        end
    end)
...
RegisterServerEvent('pepe-multichar:server:select:char')
AddEventHandler('pepe-multichar:server:select:char', function(CitizenId)
    local src = source
    if Framework.Player.Login(src, false, CitizenId) then
        TriggerClientEvent('pepe-spawn:client:choose:spawn', src)
        TriggerEvent("pepe-logs:server:SendLog", "joinleave", "Loaded", "green", "**".. GetPlayerName(src) .. "** ("..CitizenId.." | "..src..") loaded..")
	end
end)

RegisterServerEvent('pepe-multichar:server:create:new:char')
AddEventHandler('pepe-multichar:server:create:new:char', function(NewCharData)
    local src = source
    ...
    if Framework.Player.Login(src, true, false, newData) then
        ...
        TriggerEvent("pepe-logs:server:SendLog", "joinleave", "Character Creation", "green", "**".. GetPlayerName(src) .. "** ("..src..") Created their character..")
	end
end)

RegisterServerEvent('pepe-multichar:server:delete:char')
AddEventHandler('pepe-multichar:server:delete:char', function(CitizenId)
