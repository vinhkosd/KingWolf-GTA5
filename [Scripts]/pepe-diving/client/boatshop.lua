...
function SpawnBerthBoats()
    for loc,_ in pairs(QBBoatshop.Locations["berths"]) do
        if SpawnedBoats[loc] ~= nil then
            Framework.Functions.DeleteVehicle(SpawnedBoats[loc])
        end
    ...
    while true do
        ...
        if distance < 15 then
            ...
            if BuyDistance < 2 then                
                ...
                if not Buying then
                    DrawText3D(BuyLocation.x, BuyLocation.y, BuyLocation.z + 0.3, '~g~E~w~ - '..QBBoatshop.ShopBoats[currentBoat]["label"]..' Mua thuyền ~b~€'..QBBoatshop.ShopBoats[currentBoat]["price"])
                    if IsControlJustPressed(0, Keys["E"]) then
                        Buying = true
                    end
                else
                    DrawText3D(BuyLocation.x, BuyLocation.y, BuyLocation.z + 0.3, 'Bạn có chắc không? ~g~7~w~ Đồng ý / ~r~8~w~ Từ chối ~b~($'..QBBoatshop.ShopBoats[currentBoat]["price"]..',-)')
                    if IsControlJustPressed(0, Keys["7"]) or IsDisabledControlJustReleased(0, Keys["7"]) then
                        TriggerServerEvent('pepe-diving:server:BuyBoat', QBBoatshop.Locations["berths"][ClosestBerth]["boatModel"], ClosestBerth)
                        Buying = false
...
RegisterNetEvent('pepe-diving:client:BuyBoat')
AddEventHandler('pepe-diving:client:BuyBoat', function(boatModel, plate)
    DoScreenFadeOut(250)
    Citizen.Wait(250)
...
Citizen.CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Thuê thuyền")
