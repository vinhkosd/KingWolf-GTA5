...
function UpdateXP(_xp, init)
    ...
    TriggerServerEvent("kingwolf-exp:setXP", points, rank)
end
...
function ESXP_SetInitial(XPInit)
    ...
    if not GoalXP or (GoalXP < 0 or GoalXP > ESXP_GetMaxXP()) then
        TriggerEvent("kingwolf-exp:print", _('err_xp_update', XPInit, "ESXP_SetInitial"))
        return
    end    
...
function ESXP_SetRank(Rank)
    ...
    if not GoalRank then
        TriggerEvent("kingwolf-exp:print", _('err_lvl_update', Rank, "ESXP_SetRank"))
        return
    end
...
function ESXP_Add(XPAdd)
    ...
    if not tonumber(XPAdd) then
        TriggerEvent("kingwolf-exp:print", _('err_xp_update', XPAdd, "ESXP_Add"))
        return
    end       
...
function ESXP_Remove(XPRemove)
    ...
    if not tonumber(XPRemove) then
        TriggerEvent("kingwolf-exp:print", _('err_xp_update', XPRemove, "ESXP_Remove"))
        return
    end       
...
function ESXP_GetXPToRank(Rank)
    ...
    if not GoalRank or (GoalRank < 1 or GoalRank > #Config.Ranks) then
        TriggerEvent("kingwolf-exp:print", _('err_lvl_update', Rank, "ESXP_GetXPToRank"))
        return
    end
