...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if NearATM() and not isBankOpened and NearATM() ~= "update" then
            ...
            if IsControlJustReleased(0, 38) then
            	Framework.Functions.TriggerCallback("okokBanking:GetPIN", function(pin)
            		if pin then
...
function openBank()
	local hasJob = false
	local playeJob = Framework.Functions.GetPlayerData().job
	local playerJobName = ''
	...
	if playeJob ~= nil then
		...
		playerJobGrade = playeJob.isboss
		jobLabel = playeJob.label
		society = 'society_'..playerJobName
	end

	Framework.Functions.TriggerCallback("okokBanking:GetPlayerInfo", function(data)
		Framework.Functions.TriggerCallback("okokBanking:GetOverviewTransactions", function(cb, identifier, allDays)
			...
			if canAccessSociety then
				Framework.Functions.TriggerCallback("okokBanking:SocietyInfo", function(cb)
					if cb ~= nil then
						societyInfo = cb
					else
			...
			Framework.Functions.TriggerCallback("okokBanking:GetSocietyTransactions", function(societyTranscb, societyID, societyAllDays)
...
RegisterNUICallback("action", function(data, cb)
	...
	elseif data.action == "transfer" then
		if tonumber(data.value) ~= nil then
			if tonumber(data.value) > 0 then
				Framework.Functions.TriggerCallback("okokBanking:IsIBanUsed", function(isUsed, isPlayer)
					if isUsed ~= nil then
						if data.window == 'bankmenu' then
							if isPlayer then
								TriggerServerEvent('okokBanking:TransferMoney', tonumber(data.value), data.iban:upper(), isUsed.citizenid, isUsed.money, isUsed.name)
						...
						elseif data.window == 'societies' then
							...
							if isPlayer then
								TriggerServerEvent('okokBanking:TransferMoneyToPlayerFromSociety', tonumber(data.value), data.iban:upper(), isUsed.citizenid, isUsed.money, isUsed.name, societyInfo.society, societyInfo.society_name, societyInfo.value, toMyself)
							elseif not isPlayer then
...
AddEventHandler("okokBanking:updateTransactions", function(money, wallet)
	...
	if isBankOpened then
		Framework.Functions.TriggerCallback("okokBanking:GetOverviewTransactions", function(cb, id, allDays)
			trans = cb
...
AddEventHandler("okokBanking:updateIbanPinChange", function()
	Citizen.Wait(100)
	Framework.Functions.TriggerCallback("okokBanking:GetOverviewTransactions", function(cbs, ids, allDays)
		trans = cbs
	end)
end)
...
function GenerateIBAN()
	...
	while true do
		...
		if isIBanUsed and not hasChecked then
			isIBanUsed = false
			Framework.Functions.TriggerCallback("okokBanking:IsIBanUsed", function(isUsed)
				if isUsed ~= nil then
					isIBanUsed = true
