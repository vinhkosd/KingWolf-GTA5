...
RegisterNUICallback('CloseApp', function()
    ...
    Framework.Functions.Progressbar("bank", "Lấy thẻ ...", 5000, false, false, {
...
AddEventHandler('pepe-banking:client:check:players:near', function(TargetPlayer, Amount)
    ...
    if Player ~= -1 and Distance < 3.0 then
        if GetPlayerServerId(Player) == TargetPlayer then
            ...
            Framework.Functions.Notify("Người chơi không chính xác.", "error")
        end
    else
        Framework.Functions.Notify("Không tìm thấy người chơi.", "error")
    end
end)
...
function OpenBank(CanDeposit, UseAnim)
    ...
    Framework.Functions.Progressbar("bank", "Cho thẻ vào..", 4500, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
