	...
	if Source ~= nil then
		...
		Framework.Functions.ExecuteSql(false, "SELECT * FROM `characters_metadata` WHERE `citizenid` = '"..CitizenId.."'", function(result)
		 	...
		 	if PlayerData ~= nil then
				...
				print("PlayerData is nil CitizenId: '"..CitizenId.."'")
			end	
			Framework.Player.CheckPlayerData(Source, PlayerData)		
		 end)
		 return true
...
Framework.Player.CheckPlayerData = function(source, PlayerData)
	...
	PlayerData.metadata["robberynexttime"] = PlayerData.metadata["robberynexttime"] ~= nil and PlayerData.metadata["robberynexttime"] or 0
	PlayerData.metadata["lastrobberycoords"] = PlayerData.metadata["lastrobberycoords"] ~= nil and PlayerData.metadata["lastrobberycoords"] or nil
...
Framework.Player.CreatePlayer = function(PlayerData)
	...
	self.Functions.AddMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] ~= nil then
			...
			if amount > 50000 then
				TriggerEvent("pepe-logs:server:SendLog", "playermoney", "AddMoney", "lightgreen", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** $"..amount .. " ("..moneytype..") Đã nhận "..moneytype.." số tiền: $"..self.PlayerData.money[moneytype], true)
			else
				TriggerEvent("pepe-logs:server:SendLog", "playermoney", "AddMoney", "lightgreen", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** $"..amount .. " ("..moneytype..") đã nhận "..moneytype.." số tiền: $"..self.PlayerData.money[moneytype])
			end
	...
	self.Functions.RemoveMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] ~= nil then
			...
			if amount > 100000 then
				TriggerEvent("pepe-logs:server:SendLog", "playermoney", "RemoveMoney", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** €"..amount .. " ("..moneytype..") đã trừ "..moneytype.." số tiền: "..self.PlayerData.money[moneytype].." reason:"..reason, true)
			else
				TriggerEvent("pepe-logs:server:SendLog", "playermoney", "RemoveMoney", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** €"..amount .. " ("..moneytype..") đã trừ "..moneytype.." số tiền: "..self.PlayerData.money[moneytype].." reason:"..reason)
			end
			-- if moneytype == 'cash' then
			-- 	TriggerClientEvent("pepe-hud:client:money:change", self.PlayerData.source, amount, 'Minus')
			-- end
	...
	self.Functions.SetMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] ~= nil then
			...
			self.Functions.UpdatePlayerData()
			TriggerEvent("pepe-logs:server:SendLog", "playermoney", "SetMoney", "green", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** đã set $"..amount .. " ("..moneytype..") , "..moneytype.." mới: "..self.PlayerData.money[moneytype])
			return true
		end
		return false
	end
	...
	self.Functions.AddItem = function(item, amount, slot, info)
		...
		if (totalWeight + (itemInfo["weight"] * amount)) <= Framework.Config.Player.MaxWeight then
			if (slot ~= nil and self.PlayerData.items[slot] ~= nil) and (self.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo["type"] == "item" and not itemInfo["unique"]) then
				...
				self.Functions.UpdatePlayerData()
				TriggerEvent("pepe-logs:server:SendLog", "playerinventory", "AddItem", "green", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** đã nhận: [slot:" ..slot.."], itemname: " .. amount .."x" .. self.PlayerData.items[slot].name .. ", tổng số lượng đang có: ".. self.PlayerData.items[slot].amount)
				return true
			...
			elseif (itemInfo["unique"]) or (not slot or slot == nil) or (itemInfo["type"] == "weapon") then
				for i = 1, Config.Player.MaxInvSlots , 1 do
					if self.PlayerData.items[i] == nil then
						...
						self.Functions.UpdatePlayerData()
						TriggerEvent("pepe-logs:server:SendLog", "playerinventory", "AddItem", "green", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** đã nhận: [slot:" ..i.."], itemname: " .. amount .."x" .. self.PlayerData.items[i].name .. ", tổng số lượng đang có: ".. self.PlayerData.items[i].amount)
						return true
					end
				end
			end
	...
	self.Functions.RemoveItem = function(item, amount, slot)
		...
		if slot ~= nil then
			if self.PlayerData.items[slot].amount > amount then
				self.PlayerData.items[slot].amount = self.PlayerData.items[slot].amount - amount
				self.Functions.UpdatePlayerData()
				TriggerEvent("pepe-logs:server:SendLog", "playerinventory", "RemoveItem", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** đã xóa item: [slot:" ..slot.."], itemname: " .. amount .."x" .. self.PlayerData.items[slot].name .. ", số lượng còn lại: ".. self.PlayerData.items[slot].amount)
				return true
			else
				self.PlayerData.items[slot] = nil
				self.Functions.UpdatePlayerData()
				TriggerEvent("pepe-logs:server:SendLog", "playerinventory", "RemoveItem", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** đã xóa item: [slot:" ..slot.."], itemname: " .. amount .."x" .. item .. "")
				return true
			end
		else
			...
			if slots ~= nil then
				for _, slot in pairs(slots) do
					...
					elseif self.PlayerData.items[slot].amount == amountToRemove then
						self.PlayerData.items[slot] = nil
						self.Functions.UpdatePlayerData()
						TriggerEvent("pepe-logs:server:SendLog", "playerinventory", "RemoveItem", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** đã xóa item: [slot:" ..slot.."], itemname: " .. amount .."x" .. item .. ", item đã xóa khỏi slot")
						return true
					end
				end
			end
	...
	self.Functions.ClearInventory = function()
		...
		TriggerEvent("pepe-logs:server:SendLog", "playerinventory", "ClearInventory", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** đã clear inventory")
	end
	...
	local otherPlayerByCitizenId = Framework.Functions.GetPlayerByCitizenId(self.PlayerData.citizenid)
	if(otherPlayerByCitizenId ~= nil) then
		print('LoginSameTimes'..self.PlayerData.citizenid)
		Framework.Functions.Kick(self.PlayerData.source, "Tài khoản đăng nhập nhiều thiết bị!", nil, nil)
		Framework.Functions.Kick(otherPlayerByCitizenId.PlayerData.source, "Tài khoản đăng nhập nhiều thiết bị!", nil, nil)
		if(self.PlayerData.source~=nil and (Framework.Players[self.PlayerData.source] ~= nil)) then 
			Framework.Players[self.PlayerData.source] = nil
		end

		if(otherPlayerByCitizenId.PlayerData.source~=nil and (Framework.Players[otherPlayerByCitizenId.PlayerData.source] ~= nil)) then 
			Framework.Players[otherPlayerByCitizenId.PlayerData.source] = nil
		end
		
		
		-- DropPlayer(self.PlayerData.source, "Tài khoản đăng nhập nhiều thiết bị!")
		-- DropPlayer(otherPlayerByCitizenId.PlayerData.source, "Tài khoản đăng nhập nhiều thiết bị!")
		return nil
	end
...
Framework.Player.Save = function(source)
	...
	if PlayerData ~= nil then
		...
		print('SAVE DATA ERROR - NILL - pepe-CORE')
	end
end
...
Citizen.CreateThread(function()
	print("\x1b[32m[pepe-core:LOG]\x1b[0m player.lua")
end)