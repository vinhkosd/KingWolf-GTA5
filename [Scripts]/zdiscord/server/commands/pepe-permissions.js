...
module.exports = {
    ...
    run: async (client, interaction, args) => {
        return interaction.reply({ content: "Tính năng đang không thể sử dụng", ephemeral: false });
        // if (!GetPlayerName(args.id)) return interaction.reply({ content: "ID Người chơi không hợp lệ.", ephemeral: true });
        // if (args.add) {
        //     client.Framework.Functions.AddPermission(args.id, args.permission);
        ...
        // } else if (args.remove) {
        //     client.Framework.Functions.RemovePermission(args.id);
