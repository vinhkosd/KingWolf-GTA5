    ...
    while true do
        ...
        if LoggedIn then
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].outsideLocation.x, Config['delivery'].outsideLocation.y, Config['delivery'].outsideLocation.z, true) < 1.3 then
                ...
                DrawText3D(Config['delivery'].outsideLocation.x, Config['delivery'].outsideLocation.y, Config['delivery'].outsideLocation.z + 0.15, "~g~E~w~ - Vào trong")
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].insideLocation.x, Config['delivery'].insideLocation.y, Config['delivery'].insideLocation.z, true) < 15 and not IsPedInAnyVehicle(GetPlayerPed(-1), false) and not onDuty then
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].insideLocation.x, Config['delivery'].insideLocation.y, Config['delivery'].insideLocation.z, true) < 1.3 then
                    DrawText3D(Config['delivery'].insideLocation.x, Config['delivery'].insideLocation.y, Config['delivery'].insideLocation.z + 0.15, "~g~E~w~ - Ra ngoài")
                    if IsControlJustReleased(0, 38) then
                        DoScreenFadeOut(500)
                        while not IsScreenFadedOut() do
                            Citizen.Wait(10)
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, 1049.15,-3100.63,-39.95, true) < 15 and not IsPedInAnyVehicle(GetPlayerPed(-1), false) and carryPackage == nil then
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, 1049.15,-3100.63,-39.95, true) < 1.3 then
                    if onDuty then
                        DrawText3D(1049.15,-3100.63,-39.95 + 1.15, "~g~E~w~ - Ra ca")
                    else
                        DrawText3D(1049.15,-3100.63,-39.95 + 1.15, "~g~E~w~ -  Vào ca")
                    end
                    if IsControlJustReleased(0, 38) then
                        onDuty = not onDuty
                        if onDuty then
                            TriggerEvent('pepe-phone:client:induty', source, "Bạn đang trong ca trực!")
                            -- Framework.Functions.Notify("Je bent ingeklokt")
                        else
                            TriggerEvent('pepe-phone:client:offduty', source, "Bạn đã hết ca trực!")
                ...
                if packagePos ~= nil then
                    ...
                    if carryPackage == nil then
                        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, packagePos.x,packagePos.y,packagePos.z, true) < 2.3 then
                            DrawText3D(packagePos.x,packagePos.y,packagePos.z+ 1, "~g~E~w~ - Lấy thùng tái chế")
                            if IsControlJustReleased(0, 38) then
                                TaskStartScenarioInPlace(GetPlayerPed(-1), "PROP_HUMAN_BUM_BIN", 0, true)
                                Framework.Functions.Progressbar("pickup_reycle_package", "Đang lấy thùng tái chế..", 5000, false, true, {
                            ...
                            DrawText3D(packagePos.x, packagePos.y, packagePos.z + 1, "Thùng tái chế")
                        end
                    else
                        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z, true) < 2.0 then
                            DrawText3D(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z, "~g~E~w~ - Tái chế")
                            if IsControlJustReleased(0,38) then
                                DropPackage()
                                ScrapAnim()
                                Framework.Functions.Progressbar("deliver_reycle_package", "Đang tái chế..", 5000, false, true, {
                                    disableMovement = true,
                                    ...
                                    TriggerServerEvent('pepe-materials:server:recycle:reward')
                                    GetRandomPackage()
                                end)
                            end
                        else
                            DrawText3D(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z, "Nơi tái chế")
                        end
                    end
                else
                    GetRandomPackage()
