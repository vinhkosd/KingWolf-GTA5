...
RegisterNetEvent('pepe-prison:client:spawn:prison')
AddEventHandler('pepe-prison:client:spawn:prison', function()
  Citizen.SetTimeout(550, function()
    Framework.Functions.GetPlayerData(function(PlayerData)
     local RandomStartPosition = Config.Locations['Spawns'][math.random(1, #Config.Locations['Spawns'])]
     TriggerEvent('pepe-sound:client:play', 'jail-door', 0.5)
     Citizen.Wait(450)
...
RegisterNetEvent('pepe-prison:client:enter:prison')
AddEventHandler('pepe-prison:client:enter:prison', function(Time, bool)
    JailTime = Time
    InJail = bool
end)

RegisterNetEvent('pepe-prison:client:set:alarm')
AddEventHandler('pepe-prison:client:set:alarm', function(bool)
  if bool then
...
RegisterNetEvent('pepe-prison:client:leave:prison')
AddEventHandler('pepe-prison:client:leave:prison', function()
  local RandomSeat = Config.Locations['Leave-Spawn'][math.random(1, #Config.Locations['Leave-Spawn'])]
  DoScreenFadeOut(1000)
  Citizen.Wait(1000)
  TriggerEvent('pepe-sound:client:play', 'jail-cell', 0.2)
      ...
      if isLoggedIn then
        if InJail then
          if JailTime > 0 then
            ...
            JailTime = JailTime - 1
            TriggerServerEvent("pepe-prison:server:set:jail:state", JailTime)
           ...
           if Distance < 2.5 then
            if JailTime <= 0 then
              ...
              if IsControlJustReleased(0, 38) then
                InJail = false
                AlertSended = false
                TriggerServerEvent("pepe-prison:server:get:items:back")
                TriggerServerEvent("pepe-prison:server:set:jail:leave")
                TriggerEvent('pepe-prison:client:leave:prison')
              end
            else
           ...
           local Distance = GetDistanceBetweenCoords(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, v['Coords']['X'], v['Coords']['Y'], v['Coords']['Z'], true)
            ...
            local Distance = GetDistanceBetweenCoords(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, Config.Locations['Shop']['Coords']['X'], Config.Locations['Shop']['Coords']['Y'], Config.Locations['Shop']['Coords']['Z'], true)
              if Distance < 2.5 then
                ...
                if IsControlJustReleased(0, 38) then
                  TriggerServerEvent("pepe-inventory:server:OpenInventory", "shop", "prison", Config.Items)
                end
                InRange = true
              end
