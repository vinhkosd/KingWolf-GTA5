    ...
    run: async (client, interaction, args) => {
        ...
        if (args.set) {
            if (player.Functions.SetJob(args.job, args.grade)) {
                ...
                return interaction.reply({ content: `${GetPlayerName(args.id)} (${args.id}) đã chuyển ngành từ ${prevJob} thành ${args.job} (${args.grade})`, ephemeral: false });
            } else {
            ...
            return interaction.reply({ content: `${GetPlayerName(args.id)} (${args.id}) đã bị sa thải khỏi ngành ${prevJob}`, ephemeral: false });
        } else if (args.inspect) {
            return interaction.reply({ content: `${GetPlayerName(args.id)} (${args.id}) nghề nghiệp hiện tại: ${prevJob}`, ephemeral: false });
        }
    },
};
