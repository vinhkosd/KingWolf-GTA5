...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            if (PlayerJob ~= nil) and PlayerJob.name == "cardealer" and onDuty then
                if IsPedInAnyVehicle(ped, false) then
                    if dist < 15 then
                        ...
                        if dist < 2 then
                            DrawText3Ds(bringcoords.x, bringcoords.y, bringcoords.z, '~g~E~w~ - Cất xe chạy thử')
...
CustomVehicleShop = {
	...
	menu = {
		...
		["main"] = {
			...
			name = "main",
			buttons = {
				{name = "xe-compacts", description = ""},
                -- {name = "xe-coupes", description = ""},
                {name = "xe-sedans", description = ""},
                {name = "xe-vans", description = ""},
                {name = "xe-sports", description = ""},
                {name = "xe-may", description = ""},
                {name = "xe-suvs", description = ""},
                {name = "xe-offroad", description = ""},
                {name = "xe-muscle", description = ""},
                {name = "xe-cycles", description = ""},
        ...
        if isLoggedIn then
            if dist < 2 then
                if PlayerJob ~= nil then
                    if PlayerJob.name == "cardealer" and onDuty then
                        DrawText3Ds(PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.x, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.y, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.9, '~g~G~w~ - Đổi xe')
                        -- DrawText3Ds(PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.x, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.y, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.6, 'Giá: ($'..Framework.Shared.Vehicles[PepeCustoms.ShowroomPositions[ClosestCustomVehicle].vehicle].price..',-)')
                        DrawText3Ds(PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.x, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.y, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.75, '~b~/sellvh [id]~w~ - Bán xe ~b~/testrit~w~ - Chạy thử')
                                        ...
                                        if selected then
                                            if IsControlJustPressed(1, 18) then
                                                if CustomModelLoaded then
                                                    TriggerServerEvent('pepe-vehicleshop:server:SetCustomShowroomVeh', button.model, ClosestCustomVehicle)
                                                end
                                            end
                                        end
                                    end
                            ...
                            if PepeCustoms.ShowroomPositions[ClosestCustomVehicle] ~= nil then
                                if PepeCustoms.ShowroomPositions[ClosestCustomVehicle].buying then
                                    if Framework.Shared.Vehicles[PepeCustoms.ShowroomPositions[ClosestCustomVehicle].vehicle] ~= nil then
                                        DrawText3Ds(PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.x, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.y, PepeCustoms.ShowroomPositions[ClosestCustomVehicle].coords.z + 1.6, '~g~7~w~ - Đồng ý / ~r~8~w~ - Từ Chối - ~g~($'..Framework.Shared.Vehicles[PepeCustoms.ShowroomPositions[ClosestCustomVehicle].vehicle].price..',-)')
                                        
                                        if IsDisabledControlJustPressed(0, Keys["7"]) then
                                            TriggerServerEvent('pepe-vehicleshop:server:ConfirmVehicle', PepeCustoms.ShowroomPositions[ClosestCustomVehicle])
                                            PepeCustoms.ShowroomPositions[ClosestCustomVehicle].buying = false
                                        end

                                        if IsDisabledControlJustPressed(0, Keys["8"]) then
                                            Framework.Functions.Notify('Bạn đã không mua xe!')
                                            PepeCustoms.ShowroomPositions[ClosestCustomVehicle].buying = false
                                        end
                                    end
                                end
...
RegisterNetEvent('pepe-vehicleshop:client:SetCustomShowroomVeh')
AddEventHandler('pepe-vehicleshop:client:SetCustomShowroomVeh', function(showroomVehicle, k)
    CancelEvent()
...
RegisterNetEvent('pepe-vehicleshop:client:ConfirmVehicle')
AddEventHandler('pepe-vehicleshop:client:ConfirmVehicle', function(Showroom, plate)
...
RegisterNetEvent('pepe-vehicleshop:client:DoTestrit')
AddEventHandler('pepe-vehicleshop:client:DoTestrit', function(plate)
...
RegisterNetEvent('pepe-vehicleshop:client:SellCustomVehicle')
AddEventHandler('pepe-vehicleshop:client:SellCustomVehicle', function(TargetId)
    local ped = GetPlayerPed(-1)
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if VehicleDist < 2.5 then
            TriggerServerEvent('pepe-vehicleshop:server:SellCustomVehicle', TargetId, ClosestCustomVehicle)
        else
            Framework.Functions.Notify("Bạn không ở gần xe!", "error")
        end
    else
...
RegisterNetEvent('pepe-vehicleshop:client:SetVehicleBuying')
AddEventHandler('pepe-vehicleshop:client:SetVehicleBuying', function(slot)
...
function CustomButtonSelected(button)
	...
	if this == "main" then
		if btn == "xe-compacts" then
			OpenCustomMenu('compacts')
        elseif btn == "Donaties" then
            OpenCustomMenu('donos')
        elseif btn == "xe-coupes" then
            OpenCustomMenu('coupes')
        elseif btn == "xe-sedans" then
            OpenCustomMenu('sedans')
        elseif btn == "xe-vans" then
            OpenCustomMenu('vans')
        elseif btn == "xe-sports" then
            OpenCustomMenu('sports')
        elseif btn == "xe-may" then
            OpenCustomMenu('motorcycles')
        elseif btn == "xe-suvs" then
            OpenCustomMenu('suvs')
        elseif btn == "xe-offroad" then
            OpenCustomMenu('off-road')
        elseif btn == "xe-muscle" then
            OpenCustomMenu('muscle')
        elseif btn == "xe-cycles" then
            OpenCustomMenu('cycles')
