...
VolumeCheck = function(id)
    if data[id]['DUI'] then
        if data[id]['ActualVolume'] ~= data[id]['Volume'] then
            ...
            SendDuiMouseMove(duiLong, 78 + 75, 701)
            Wait(250)
            SendDuiMouseMove(duiLong, 78 + 95 + math.ceil(data[id]['Volume'] * 5), 703)
            Wait(5)
            SendDuiMouseDown(duiLong, 'left')
            Wait(7)
            SendDuiMouseUp(duiLong, 'left')

            SendDuiMouseMove(duiLong, 78 + 75, 500)
...
CreateThread(function()
    ...
    print(HasScaleformMovieLoaded(SFHandle))
            ...
            for k, v in pairs(Config['Objects']) do
                ...
                if DoesEntityExist(obj) then
                    ...
                    while #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(obj)) <= 2.0 do
                        ...
                        if playing then
                            AddTextEntry(GetCurrentResourceName(), "<FONT FACE='arial font'>"..Strings['VolumeHelp'].."</FONT>")
                            ...
                            AddTextEntry(GetCurrentResourceName(), "<FONT FACE='arial font'>"..Strings['VideoHelp'].."</FONT>")
                        end
