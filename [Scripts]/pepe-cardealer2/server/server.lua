    ...
    Citizen.SetTimeout(1000, function()
        Framework.Functions.ExecuteSql(false, "SELECT * FROM `server-cars` WHERE `type` = 'second'", function(result)
            for k, v in pairs(result) do
                ...
                TriggerClientEvent('pepe-cardealer2:client:set:premium:details', -1, v.vehiclename, v.price, v.vehicledisplayname, v.stock)
                Citizen.Wait(3)
            end
        end)
    end)
...
Framework.Functions.CreateCallback("pepe-cardealer2:server:get:config", function(source, cb)
    cb(Config)
end)

Framework.Functions.CreateCallback("pepe-cardealer2:server:get:storage:cars", function(source, cb)
    local CarsTable = {}
...
RegisterServerEvent('pepe-cardealer2:server:set:premium:data')
AddEventHandler('pepe-cardealer2:server:set:premium:data', function(Slot, Price, Model, DisplayName, Stock)
 ...
 TriggerClientEvent('pepe-cardealer2:client:sync:premium:dealer', -1, Slot, Config.PremiumDealerSpots[Slot])
end)

RegisterServerEvent('pepe-cardealer2:server:sell:closest')
AddEventHandler('pepe-cardealer2:server:sell:closest', function(TargetPlayer, CurrentSlot)
    local TargetSource = Framework.Functions.GetPlayer(TargetPlayer)
    if TargetSource ~= nil then
        TriggerClientEvent('pepe-cardealer2:client:can:buy:vehicle', TargetSource.PlayerData.source, CurrentSlot)
    end
end)

RegisterServerEvent('pepe-cardealer2:server:buy:current:vehicle')
AddEventHandler('pepe-cardealer2:server:buy:current:vehicle', function(CurrentSlot)
    local src = source
    ...
    if Config.PremiumDealerSpots[CurrentSlot]['Stock'] > 0 then
        if Player.Functions.RemoveMoney("bank", Config.PremiumDealerSpots[CurrentSlot]['Price'], "vehicle-shop") then
            ...
            TriggerClientEvent('pepe-cardealer2:client:set:stock', -1, NewStock, CurrentSlot)
            TriggerClientEvent('pepe-cardealer2:client:spawn:car:premium', src, Config.PremiumDealerSpots[CurrentSlot]['Model'], Plate, NewStock, CurrentSlot)
        else
