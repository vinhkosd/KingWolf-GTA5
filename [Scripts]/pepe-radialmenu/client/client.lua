...
AddEventHandler("pepe-radialmenu:client:send:panic:button",function()
  Framework.Functions.TriggerCallback('Framework:HasItem', function(HasItem)
      if HasItem then
          ...
          TriggerServerEvent('pepe-police:server:send:alert:panic:button', GetEntityCoords(GetPlayerPed(-1)), StreetLabel, Info)
      else
...
AddEventHandler("pepe-radialmenu:client:send:down",function(Type)
    ...
    TriggerServerEvent('pepe-police:server:send:alert:officer:down', GetEntityCoords(GetPlayerPed(-1)), StreetLabel, Info, Priority)
end)
